...
class SaleSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach (range(1, 250) as $index) {
            ...
            $baseCost = $product->cost_price > 0 ? $product->cost_price : $sellingPrice * $faker->randomFloat(2, 0.70, 0.90);
            $totalProfit = ($sellingPrice - $baseCost) * $qty;
            Sale::create(['product_id' => $product->id, 'quantity' => $qty, 'cost_price' => $baseCost, 'selling_price' => $sellingPrice, 'total_profit' => $totalProfit, 'transaction_date' => $transactionDate, 'created_at' => $transactionDate, 'updated_at' => $transactionDate,]);
        }
